...
#%% Generate 
Single_Folder_Processor(wp,subfolder='Preprocessed',save_format='python',keepna=True)
#%% let's see the frame drop problems here.
a = np.load(r'D:\ZR\_Data_Temp\Ois200_Data\red_speckle\Preprocessed\red.npy')
b = np.load(r'D:\ZR\_Data_Temp\Ois200_Data\red_speckle\Preprocessed\Speckle.npy')

plt.plot(a.sum(-1).sum(-1))
plt.plot(b.sum(-1).sum(-1))